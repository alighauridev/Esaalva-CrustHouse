...
const Bills = () => {
  ...
  const getAllBills = async () => {
    try {
      ...
      const { data } = await axios.get("/api/v1/sales-order");
      if (data) {
        const formattedData = data.map((bill) => ({
          _id: bill._id,
          customerName: bill.customer_id.name,
          customerPhone: bill.customer_id.phone,
          customerAddress: bill.customer_id.address,
          totalAmount: `${bill.amount}PKR`,
          // totalAmount: bill.amount,
          time: moment(bill.time_initiated).fromNow(),
        }));
